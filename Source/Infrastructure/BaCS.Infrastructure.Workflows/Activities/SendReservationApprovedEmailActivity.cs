...
[Activity("BaCS", "Reservation", "Send reservation approved notification to user")]
public class SendReservationApprovedEmailActivity : CodeActivity
{
    ...
    protected override async ValueTask ExecuteAsync(ActivityExecutionContext context)
    {
        ...
        if (reservationId == Guid.Empty)
        {
            ...
            context.Fault(
                new BusinessRulesException(
                    $"{Variables.ReservationId} input to execute activity {nameof(SendReservationApprovedEmailActivity)} was not found"
                )
            );
        ...
        context.AddExecutionLogEntry(
            "Info",
            $"Reservation approved email for reservation {reservationId} has been successfully sent to user"
        );
    }
}
